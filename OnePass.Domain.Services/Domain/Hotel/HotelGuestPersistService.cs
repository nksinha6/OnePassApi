...
namespace OnePass.Domain.Services
{
        ...
        IConfiguration configuration) : IHotelGuestPersistService
    {
        ...
        public async Task<HotelGuest> UpdateAadharData(UpdateAadharStatusParam param) {
            ...
            return await _guestRepository.UpdatePartialAsync(new HotelGuest() { Id = guest.Id,  FullName = param.Name, DateOfBirth = param.DateOfBirth, Gender = param.Gender, Nationality = param.Nationality, Uid =  param.Uid, VerificationId = param.VerificationId, ReferenceId = param.ReferenceId,
                
                SplitAddress = System.Text.Json.JsonSerializer.Serialize(param.SplitAddress) }, x => x.FullName, x => x.DateOfBirth, x => x.Gender, x => x.Nationality, x => x.Uid, x => x.VerificationId, x => x.ReferenceId, x => x.SplitAddress);
        }
